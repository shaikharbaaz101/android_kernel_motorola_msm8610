...
#else
static struct notifier_block smartmax_nb;
#endif
...
#else
static int smartmax_panel_cb(struct notifier_block *nb,
...
#else
			smartmax_nb.notifier_call = smartmax_panel_cb;
			if (!lcd_register_client(&smartmax_nb))
				pr_info("Smartmax: registered lcd notifier\n");
#endif
...
#else
			lcd_unregister_client(&smartmax_nb);
#endif
		}
